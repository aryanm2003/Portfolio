...
import { ArrowLeft, Trash2, PlusCircle, Edit, CheckCircle, XCircle, AlertCircle, Monitor, Users } from 'lucide-react';
...
const EditCourses = () => {
    ...
    useEffect(() => {
        if (selectedCourseId) {
            ...
            if (selectedCourse) setFormData(selectedCourse);
        } else {
            setFormData({ title: '', content: '', image: '', docLink: '', category: 'online' });
        }
    ...
    const handleUpdate = async (e) => {
        ...
        const token = localStorage.getItem('adminToken');
        try {
            const res = await fetch(`http://localhost:5000/api/courses/${selectedCourseId}`, {
                method: 'PUT',
                headers: { 
                    'Content-Type': 'application/json', 
                    'Authorization': `Bearer ${token}` 
