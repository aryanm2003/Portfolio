...
import { ArrowLeft, Trash2, PlusCircle, Edit, CheckCircle, XCircle, AlertCircle, User, Users, Clock } from 'lucide-react';
...
const EditTeamMembers = () => {
    ...
    useEffect(() => {
        if (selectedMemberId) {
            ...
            if (selectedMember) setFormData(selectedMember);
        } else {
            setFormData({ name: '', title: '', about: '', image: '', type: 'present' });
        }
    ...
    const handleUpdate = async (e) => {
        ...
        const token = localStorage.getItem('adminToken');
        try {
            const res = await fetch(`http://localhost:5000/api/team/${selectedMemberId}`, {
                method: 'PUT',
                headers: { 
                    'Content-Type': 'application/json', 
                    'Authorization': `Bearer ${token}` 
